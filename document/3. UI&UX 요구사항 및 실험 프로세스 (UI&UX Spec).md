...
## 1. 사용자 인터페이스 레이아웃
Claude Code는 `streamlit`을 사용하여 다음 구조를 구현한다.
...
---
## ⚠️ Claude Code를 위한 특별 지시사항
  - VRAM 관리: 각 질문 생성 후 `torch.cuda.empty_cache()`를 호출하여 Colab GPU 런타임에서 OOM(Out of Memory)이 발생하지 않도록 할 것.
